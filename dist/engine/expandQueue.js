...
export class ExpandQueueService {
    ...
    async expand(req, res) {
        ...
        const errors = [];
        const queuedFens = []; // accumulates every successfully queued FEN
        const visited = new Set();
        const frontier = [{ fen: req.fen, depth: 0 }];
        outerLoop: while (frontier.length > 0) {
            ...
            if (analysis.error || !analysis.data || analysis.data.moves.length === 0) {
                ...
                if (qr.success) {
                    ...
                    queuedFens.push(fen);
                    if (res)
                        sseWrite(res, { event: 'queued', fen, positionsQueued });
                }
                else {
            ...
            for (const move of topMoves) {
                ...
                if (isLeaf) {
                    ...
                    if (qr.success) {
                        ...
                        if (res)
                            sseWrite(res, { event: 'queued', fen: childFen, positionsQueued });
                    }
                    else {

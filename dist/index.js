...
app.get('/expandqueue/stream', async (req, res) => {
    try {
        ...
        const toPositiveInt = (v, label, max) => {
            if (v === undefined)
                return undefined;
            const n = Number(v);
        ...
        await expandQueueService.expandStream({ fen, expansionDepth: depthNum, expansionWidth: widthNum, maxPositionsQueued: maxQueued }, res);
    }
    catch (error) {
